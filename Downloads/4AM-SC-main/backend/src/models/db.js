...
if (MYSQL_URL || (DATABASE_URL && DATABASE_URL.startsWith('mysql://'))) {
  ...
  async function init() {
    ...
    // Create Manager 1: Ahmed Nagi
    try {
      const manager1Check = await connection.execute('SELECT id FROM users WHERE email = ?', ['admin@4am1.com']);
      if (manager1Check[0].length === 0) {
        const bcrypt = require('bcrypt');
        const SALT_ROUNDS = Number(process.env.BCRYPT_SALT_ROUNDS || 10);
        const manager1PasswordHash = await bcrypt.hash('admin2211', SALT_ROUNDS);
        await connection.execute(
          'INSERT INTO users(name, email, password_hash, user_type, department) VALUES(?, ?, ?, ?, ?)',
          ['Mr. Ahmed Nagi', 'admin@4am1.com', manager1PasswordHash, 'admin', null]
        );
    ...
    // Create Manager 2: Ibrahim Ahmed
    try {
      const manager2Check = await connection.execute('SELECT id FROM users WHERE email = ?', ['admin@4am2.com']);
      if (manager2Check[0].length === 0) {
        const bcrypt = require('bcrypt');
        const SALT_ROUNDS = Number(process.env.BCRYPT_SALT_ROUNDS || 10);
        const manager2PasswordHash = await bcrypt.hash('admin3322', SALT_ROUNDS);
        await connection.execute(
          'INSERT INTO users(name, email, password_hash, user_type, department) VALUES(?, ?, ?, ?, ?)',
          ['Mr. Ibrahim Ahmed', 'admin@4am2.com', manager2PasswordHash, 'admin', null]
        );
  ...
  async function init() {
    ...
    // Create Manager 1: Ahmed Nagi
    try {
      const manager1Check = await pool.query('SELECT id FROM users WHERE email = $1', ['admin@4am1.com']);
      if (manager1Check.rows.length === 0) {
        const bcrypt = require('bcrypt');
        const SALT_ROUNDS = Number(process.env.BCRYPT_SALT_ROUNDS || 10);
        const manager1PasswordHash = await bcrypt.hash('admin2211', SALT_ROUNDS);
        await pool.query(
    ...
    // Create Manager 2: Ibrahim Ahmed
    try {
      const manager2Check = await pool.query('SELECT id FROM users WHERE email = $1', ['admin@4am2.com']);
      if (manager2Check.rows.length === 0) {
        const bcrypt = require('bcrypt');
        const SALT_ROUNDS = Number(process.env.BCRYPT_SALT_ROUNDS || 10);
        const manager2PasswordHash = await bcrypt.hash('admin3322', SALT_ROUNDS);
        await pool.query(
  ...
  async function init() {
    ...
    // Create Manager 1: Ahmed Nagi
    try {
      const manager1Check = await getAsync('SELECT id FROM users WHERE email = ?', ['admin@4am1.com']);
      if (!manager1Check) {
        const bcrypt = require('bcrypt');
        const SALT_ROUNDS = Number(process.env.BCRYPT_SALT_ROUNDS || 10);
        const manager1PasswordHash = await bcrypt.hash('admin2211', SALT_ROUNDS);
    ...
    // Create Manager 2: Ibrahim Ahmed
    try {
      const manager2Check = await getAsync('SELECT id FROM users WHERE email = ?', ['admin@4am2.com']);
      if (!manager2Check) {
        const bcrypt = require('bcrypt');
        const SALT_ROUNDS = Number(process.env.BCRYPT_SALT_ROUNDS || 10);
        const manager2PasswordHash = await bcrypt.hash('admin3322', SALT_ROUNDS);
